...
status_t flexspi_nor_get_vendor_id(FLEXSPI_Type *base, uint8_t *vendorId, uint16_t *deviceId)
{
    ...
    /* JEDEC ID byte order: Manufacturer(byte0), MemoryType(byte1), Capacity(byte2) */
    /* In little-endian, byte0 is at lowest address (LSB of temp) */
    *vendorId = temp & 0xFF;  /* Manufacturer ID is first byte */
    if (deviceId != NULL)
    {
        *deviceId = (temp >> 8) & 0xFFFF;  /* Memory Type and Capacity */
    }
